...
class VoiceReminderListener:
    ...
    # ------------------ Listening Loop (SoundDevice) ------------------

    def listen_loop(self):
        def callback(indata, frames, time, status):
            """This callback puts audio chunks into a queue."""
            if status:
        ...
        try:
            with sd.InputStream(callback=callback, channels=1, samplerate=44100, blocksize=1024):
                print("🎤 Listening for 'WREN' commands (SoundDevice)...")
    ...
    def parse_command(self, text: str):
        if "remind me to" in text:
            return "add", text.split("remind me to", 1)[1].strip()
    ...
    def execute_command(self, cmd, arg=None):
        ...
        if cmd == "add":
            if not arg:
                response = "⚠️ Reminder text required."
            else:
            ...
            if tasks:
                response = "📋 " + ", ".join(t["text"] for t in tasks)
            else:
